...
<div id="DiscussionForm" class="FormTitleWrapper DiscussionForm">
    ...
    if ($this->ShowCategorySelector === true) {
        ...
        echo $this->Form->categoryDropDown('CategoryID', array('Value' => val('CategoryID',
            $this->Category), 'PermFilter' => array('AllowedDiscussionTypes' => 'Article')));
        echo '</div>';
        echo '</div>';
    }
    ...
    if ($thumbnail) {
        ...
        echo '<div id="ArticleThumbnailImage"><img src="' . $imagePath . '" alt="" /></div>' .
            ...
            . url('/post/deletearticlethumbnail/' . $thumbnail->ArticleThumbnailID)
    ...
    echo $this->Form->button((property_exists($this, 'Discussion')) ? 'Save' : 'Post Article',
        array('class' => 'Button Primary DiscussionButton'));
    if (!property_exists($this, 'Discussion') || !is_object($this->Discussion) || (property_exists($this,
                'Draft') && is_object($this->Draft))
    ) {
